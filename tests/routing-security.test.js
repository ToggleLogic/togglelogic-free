...
test("the before_agent_reply gate claims the turn before any model executes", async () => {
  // Corrected architecture: the interceptor's preflight delegates entirely to
  // skillRouting.handleGate; a handled reply short-circuits with zero classifier
  // work. (No structuredPlannedSkills dependency — the coordinator resolves
  // skills from message text internally.)
  ...
  const skillRouting = {
    handleGate: async () => ({
      ...
      reason: "skill_education_required",
      audit: { mode: "skill_routing", status: "education_required" },
    }),
  };
  const interceptor = harness(normalizeConfig({ mode: "intelligence" }), { seam, skillRouting });
  assert.deepEqual(
    await interceptor.preflight({ prompt: "Prepare me for my 2 pm meeting using the meeting-prep skill." }, { sessionKey: "skill-education" }),
    {
      handled: true,
      reply: { text: "Choose a model for meeting-prep: 1, 2, or 3." },
      reason: "skill_education_required",
...
test("token choices are executed by the gate; before_model_resolve never routes token text", async () => {
  // The gate (before_agent_reply) owns the whole skill lifecycle including the
  // owner's token reply, which now executes in a bounded child. before_model_resolve
  // must NOT route on the bare token text (models are assigned to resolved skills).
  const seam = { status: () => "available", classify: async () => null };
  const skillRouting = {
    handleGate: async (event) => event.prompt === "TL-abc123 3"
      ? { handled: true, reply: { text: "briefing complete\n\n— Routed by ToggleLogic to google/gemini-flash → google/gemini-3.5-flash in a bounded child session." }, reason: "skill_choice_executed", audit: { mode: "skill_routing" } }
      : null,
  };
  const interceptor = harness(normalizeConfig({ mode: "intelligence" }), { seam, skillRouting });
  const gated = await interceptor.preflight({ prompt: "TL-abc123 3" }, { sessionKey: "skill-choice" });
  assert.equal(gated.handled, true);
  assert.equal(gated.reason, "skill_choice_executed");
  // before_model_resolve stays passthrough for the token turn (no skill override here).
  assert.deepEqual(await interceptor({ prompt: "TL-abc123 3" }, { sessionKey: "skill-choice" }), {});
});

test("a gate error on a NON-governed turn falls through (never throws into the hook)", async () => {
  const seam = { status: () => "available", classify: async () => null };
  // No evaluateScope / not active → the turn is not governed, so falling through
  // to normal routing is the safe behavior. (The REAL coordinator computes scope
  // itself and fails closed for governed turns; see the next test.)
  const skillRouting = { handleGate: async () => { throw new Error("gate exploded"); } };
  const interceptor = harness(normalizeConfig({ mode: "passthrough" }), { seam, skillRouting });
  assert.deepEqual(await interceptor.preflight({ prompt: "TL-abcdef 2" }, { sessionKey: "skill-write-error" }), { handled: false });
});

test("a gate error on a GOVERNED turn FAILS CLOSED (handled), never falls through to inline routing", async () => {
  // Finding #1: an internal gate error on an in-scope active governed turn must
  // NOT recreate the inline-execution incident path. The interceptor uses the
  // coordinator's total evaluateScope() to detect a governed turn and returns a
  // handled fail-closed error instead of unhandled passthrough.
  let classifierCalls = 0;
  const seam = { status: () => "available", classify: async () => { classifierCalls += 1; return null; } };
  const skillRouting = {
    evaluateScope: () => ({ active: true, scopeDecision: { inScope: true } }),
    handleGate: async () => { throw new Error("gate exploded"); },
  };
  const interceptor = harness(normalizeConfig({ mode: "intelligence" }), { seam, skillRouting });
  const result = await interceptor.preflight({ prompt: "run the code-review skill" }, { sessionKey: "governed-gate-error", channel: "telegram", senderId: "7797183919" });
  assert.equal(result.handled, true, "governed gate error is handled (held), not passed through");
  assert.equal(result.reason, "skill_routing_gate_error");
  assert.equal(classifierCalls, 0, "no classifier/model routing ran on the failed governed turn");
});
